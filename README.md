...
`setupJoinMate` 也會建立每 30 分鐘執行一次的排程，用 HTTPS 喚醒免費
Render 服務並檢查活動提醒。排程端點會忽略 25 分鐘內的重複呼叫，避免舊
排程持續喚醒 Neon。Email 使用資料庫 outbox，寄送成功前最多重試五次；
站內通知不受 Email 寄送失敗影響。
...
正式環境不會在伺服器內每 30 秒輪詢資料庫。Apps Script 每 30 分鐘檢查一次
即將開始的活動；正式名單會在活動前 24 小時及 1 小時收到站內提醒，同一位
成員在同一提醒階段只會收到一次。報名、成團、取消、活動修改與候補轉正的
Email 會在操作完成後立即於背景寄送，不必等待排程。

若本機開發時確實需要舊版 30 秒提醒 worker，可自行設定
`JOINMATE_ENABLE_REMINDER_WORKER=1`；Render 固定設為 `0`，避免 Neon 無法休眠。
