...
GitHub repository 並套用 `render.yaml`。部署完成後會取得固定的
`https://joinmate-....onrender.com` 網址。

本機執行仍使用 `joinmate.db`；Render 則由 `DATABASE_URL` 自動切換至
PostgreSQL，因此本機與雲端資料彼此獨立。
...
24 小時及 30 分鐘收到站內提醒；同一位成員在同一提醒階段只會收到一次。
